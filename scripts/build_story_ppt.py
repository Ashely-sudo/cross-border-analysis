...
rows2 = [
    ...
    ("问题：", "跨境电商公司到底怎么赚钱、怎么才能走得长远？平台佣金、广告、库存与现金流如何影响一家公司的盈利结构？"),
    ("猜测：", "① 行业层面：增速放缓、参与者变多，流量与广告成本可能被推高；② 模式层面：精品/铺货/长尾的费用结构不同，销售费用率对净利率影响大；③ 市场与渠道：不同市场与渠道的毛利、费用、本地化门槛存在差异，可能影响整体盈利；④ 资金层面：备货与回款占用资金，盈利与现金流未必同步"),
    ("验证：", "以 5 家上市公司为样本，用公开财报和海关数据，从行业、公司、现金流三个层面验证：哪些因素真正决定了跨境电商公司能不能赚钱、能不能走得长远。"),
]
...
qs = [
    ("① 行业", "这盘生意还有多大、还在涨吗？为什么门槛低、倒得也多？"),
    ("② 模式", "精品 / 铺货 / 长尾，谁的利润更能留下来？"),
    ("③ 市场与渠道", "北美 vs 欧洲、平台 vs 独立站，如何影响毛利与费用？"),
    ("④ 经营与资金", "为什么利润和现金流经常对不上？现金流是不是生死线，如何判断该转型？"),
]
...
# ---------------- S6 进入与退出 ----------------
s = add_slide()
header(s, "DATA · 进入与退出", "支撑数据：这个行业进得有多快、洗牌有多快")
blk6 = [
...
bullets(s, [
    ...
    ("这是跨境行业“账面赚钱、手里没现金”的典型：利润被货和应收占住了", 0, True),
